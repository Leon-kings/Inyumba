...
//     subtitle: "Inkuru Nyazo Z'abanyeshuri Babonye Inzu Nziza",
//     description:
//       "Abanyeshuri bo mu Rwanda babonye amazu meza, afite umutekano, kandi ahendutse binyuze muri INYUMBA.",
//     verified: "Umunyeshuri Wemejwe",
//     rating: "Igipimo",
...
//     shareDesc:
//       "Wabonye inzu yawe binyuze muri INYUMBA ? Tanga ubuhamya bwawe ugafasha abandi banyeshuri!",
...
// Translation helper using MyMemory API (no CORS issues)
const translateContent = async (text: string, targetLang: string): Promise<string> => {
      ...
      try {
        ...
        if (result.success) {
          ...
          if (lang !== 'en') {
            const translatedTestimonials = await Promise.all(
              testimonialsWithHelpers.map(async (item) => {
                const [translatedName, translatedUniversity, translatedTitle, translatedContent, translatedHouseName, translatedLocation] = 
                  await Promise.all([
                    translateContent(item.name, lang),
                    translateContent(item.university, lang),
                    translateContent(item.title, lang),
                    translateContent(item.content, lang),
                    item.houseName ? translateContent(item.houseName, lang) : Promise.resolve(undefined),
