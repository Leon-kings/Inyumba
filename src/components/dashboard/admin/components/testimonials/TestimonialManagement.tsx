...
export const TestimonialManagement: React.FC = () => {
  // Get language from cookies - ALL hooks must be called before any conditional returns
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingItem) {
        ...
        await handleCreateTestimonial(formData);
        showSuccessModal(
          t.success || "Success!",
          'Testimonial added successfully!',
          `${formData.name}'s testimonial has been added`
        );
      }
  ...
  // Loading state - This is now AFTER all hooks are called
  if (isFetching) {
    return (
                ...
                <div className="sticky top-0 px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between border-b border-gray-200 bg-white/95 backdrop-blur-sm rounded-t-2xl z-10">
                  <div className="flex items-center gap-2">
                    ...
                    <h2 className="text-base sm:text-xl font-semibold text-gray-900">
                      {editingItem ? t.editDetails : 'Add Testimonial'}
                    </h2>
                  </div>
                  <motion.button
