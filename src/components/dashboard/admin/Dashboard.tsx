...
//         "https://rene-inyumba-nodejs.onrender.com/auth",
//       );
//       if (!response.ok) {
...
//     const totalUsers = apiData.totalUsers;

//     // Calculate total revenue from all users' statistics
//     const totalRevenue = apiData.users.reduce(
//       (sum, user) => sum + user.statistics.totalIncome,
//       0,
//     );

//     // Calculate total bookings (using membersCount as a proxy)
//     const totalBookings = apiData.users.reduce(
//       (sum, user) => sum + user.statistics.membersCount,
//       0,
//     );
...
//       {
//         title: t.totalRevenue,
//         value: `RWF ${(totalRevenue / 1000).toFixed(1)}K`,
//         icon: <PaymentsIcon className="w-6 h-6" />,
//         change: 15.2,
//         color: "bg-orange-500",
//       },
...
//     const activeUsers = apiData.users.filter((user) => user.isActive).length;
//     const verifiedUsers = apiData.users.filter(
//       (user) => user.isEmailVerified,
//     ).length;

//     // Calculate satisfaction rate based on active users
//     const satisfactionRate =
//       apiData.totalUsers > 0
//         ? Math.round((activeUsers / apiData.totalUsers) * 100)
//         : 0;
...
//       {
//         title: t.pendingBookings,
//         value: apiData.users.filter((user) => !user.isActive).length.toString(),
//         icon: <PendingIcon className="w-5 h-5" />,
//         color: "text-yellow-500 bg-yellow-50",
//       },
//       {
...
//     if (!apiData || apiData.users.length === 0) {
//       return [
//         {
//           id: 1,
//           user: "No users found",
...
//     const recentUsers = [...apiData.users]
//       .sort(
//         (a, b) =>
...
//   // Generate top performers from API data
...
//     // Sort users by total income and get top 4
//     const topUsers = [...apiData.users]
//       .sort((a, b) => b.statistics.totalIncome - a.statistics.totalIncome)
//       .slice(0, 4);

//     return topUsers.map((user) => ({
//       name:
//         user.name.length > 15 ? `${user.name.substring(0, 15)}...` : user.name,
//       students: user.statistics.membersCount || 1,
//       houses: user.statistics.membersCount || 1,
//       revenue: `RWF ${(user.statistics.totalIncome / 1000).toFixed(1)}K`,
//       growth:
//         user.statistics.totalIncome > 0
//           ? Math.min(Math.round(user.statistics.totalIncome / 10000), 25)
//           : 0,
//     }));
//   };
...
//   // Monthly Revenue Data (keeping dummy data for charts)
...
//   // House Distribution by Category (keeping dummy data for charts)
...
//   // User Growth Data (keeping dummy data for charts)
...
export const Dashboard: React.FC = () => {
  ...
  const fetchData = async () => {
    ...
    setError(null);
    try {
      await Promise.all([fetchUserStats(), fetchBookingStats()]);
      toast.success("Data loaded successfully!");
    } catch (err) {
      const errorMessage =
        err instanceof Error ? err.message : "Failed to fetch data";
  ...
  const getStatsFromApi = () => {
    ...
    return [
      ...
      {
        title: t.totalHouses,
        value: "156", // This would come from a house stats API if available
        icon: <HomeIcon className="w-6 h-6" />,
        change: 8.3,
        color: "bg-green-500",
      },
  ...
  const getQuickStatsFromApi = () => {
    ...
    const verifiedUsers = userStats.statistics.verifiedUsers;
  ...
  const getUserGrowthData = () => {
    ...
    // Use the total users from statistics
    const totalUsers = userStats.statistics.totalUsers;
    const newUsers = userStats.statistics.newUsersLast30Days;

    // Create a realistic growth pattern based on available data
    const months = ["Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
    const baseUsers = Math.max(0, totalUsers - newUsers);

    // Distribute the new users across months with a growth pattern
  ...
  const getUserRoleDistribution = () => {
    ...
    return userStats.statistics.usersByRole.map((item) => ({
  ...
  const getTopPerformersFromApi = () => {
    ...
    // Sort users by total income and get top 4
    const topUsers = [...userStats.recentUsers]
      .sort((a, b) => b.statistics.totalIncome - a.statistics.totalIncome)
      .slice(0, 4);
  ...
  const getMonthlyBookingData = () => {
    ...
    // Fill in all months with default values if missing
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-sm text-green-500 font-medium">
                {bookingStats && bookingStats.data.totalRevenue > 0
                  ? `RWF ${(bookingStats.data.totalRevenue / 1000).toFixed(1)}K total`
                  : "+0%"}
              </span>
            </div>
            <div className="h-64">
        ...
        {/* Charts Row 2: User Role Distribution & User Growth */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
