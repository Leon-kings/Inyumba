...
//                     </div>
//                   </div>
//                   <h3 className="text-xl font-semibold text-gray-900 text-center mb-2">{t.deleteMessage}</h3>
...
// Get initial messages with translation support
const getInitialMessages = async (hostEmail: string, lang: string): Promise<HostMessage[]> => {
  const baseMessages = [
    {
      id: '1',
      propertyId: 'prop_001',
      ...
      subject: 'Inquiry about room availability',
      content: 'Hello, I am interested in booking a room at your property for the upcoming semester. Do you have any rooms available for February 2024? I would like a room with a study desk.',
      category: 'inquiry' as HostMessage['category'],
      priority: 'medium' as HostMessage['priority'],
      status: 'pending' as HostMessage['status'],
      isRead: false,
      isFlagged: true,
      isStarred: false,
      labels: ['availability', 'semester'],
    ...
    },
    {
      ...
      subject: 'Booking confirmation needed',
      content: 'I made a booking for 3 months starting February 15th. Can you please confirm the booking and send me the payment details?',
      category: 'booking' as HostMessage['category'],
      priority: 'high' as HostMessage['priority'],
      status: 'replied' as HostMessage['status'],
      isRead: true,
      isFlagged: false,
      isStarred: true,
      labels: ['confirmation', 'payment'],
    ...
    },
    {
      ...
      subject: 'Feedback on the property',
      content: 'I stayed at your property for 2 weeks and I wanted to share that I had a great experience. The room was clean and the staff were friendly.',
      category: 'feedback' as HostMessage['category'],
      priority: 'low' as HostMessage['priority'],
      status: 'read' as HostMessage['status'],
      isRead: true,
      isFlagged: false,
      isStarred: false,
      labels: ['positive', 'review'],
  ...
  // Translate initial messages if needed
  if (lang !== 'en') {
    ...
    for (const msg of baseMessages) {
      try {
        const translatedMsg = {
          ...msg,
          propertyName: await translateContent(msg.propertyName, lang),
          guestName: await translateContent(msg.guestName, lang),
          subject: await translateContent(msg.subject, lang),
          content: await translateContent(msg.content, lang),
          replyContent: msg.replyContent ? await translateContent(msg.replyContent, lang) : undefined,
          labels: await Promise.all(msg.labels.map((label: string) => translateContent(label, lang))),
          tags: await Promise.all(msg.tags.map((tag: string) => translateContent(tag, lang))),
        };
        translatedMessages.push(translatedMsg);
      } catch (err) {
        console.error('Error translating initial message:', err);
...
const getMessages = async (hostEmail: string, lang: string): Promise<HostMessage[]> => {
  const data = localStorage.getItem(STORAGE_KEY_HOST_MESSAGES);
  if (data) {
    const allMessages = JSON.parse(data);
    // Translate existing messages if language changed
    if (lang !== 'en') {
      const translatedMessages = [];
  ...
  // Initialize with initial data
  const initialMessages = await getInitialMessages(hostEmail, lang);
  localStorage.setItem(STORAGE_KEY_HOST_MESSAGES, JSON.stringify(initialMessages));
  return initialMessages;
};
...
export const HostMessageManagement: React.FC = () => {
  ...
  const [filteredMessages, setFilteredMessages] = useState<HostMessage[]>([]);
  const [searchTerm, setSearchTerm] = useState('');
  const [filterStatus, setFilterStatus] = useState<string>('all');
  const [filterCategory, setFilterCategory] = useState<string>('all');
  const [filterPriority, setFilterPriority] = useState<string>('all');
  ...
  // Load messages with translation
  useEffect(() => {
    const loadMessages = async () => {
      setIsLoading(true);
      try {
        const loadedMessages = await getMessages(userEmail, lang);
  ...
  useEffect(() => {
    const handleCookieChange = async () => {
      ...
      if (newLang !== lang) {
        ...
        const loadedMessages = await getMessages(userEmail, newLang);
        setMessages(loadedMessages);
      }
    };
  ...
  const handleCreateMessage = async () => {
    ...
    try {
      ...
      // Translate form data if needed
      let translatedGuestName = formData.guestName;
      let translatedGuestEmail = formData.guestEmail;
      let translatedSubject = formData.subject;
      let translatedContent = formData.content;
      let translatedPropertyName = formData.propertyName;

      if (lang !== 'en') {
        try {
          translatedGuestName = await translateContent(formData.guestName, lang);
          translatedGuestEmail = await translateContent(formData.guestEmail, lang);
          translatedSubject = await translateContent(formData.subject, lang);
          translatedContent = await translateContent(formData.content, lang);
          translatedPropertyName = await translateContent(formData.propertyName || 'Unknown Property', lang);
        } catch (err) {
          console.error('Error translating form data:', err);
        }
      }
      ...
      const newMessage: HostMessage = {
        id: generateId(),
        propertyId: formData.propertyId,
        propertyName: translatedPropertyName || 'Unknown Property',
        guestId: `guest_${Date.now()}`,
        guestName: translatedGuestName,
        guestEmail: translatedGuestEmail,
        guestPhone: formData.guestPhone,
  ...
  const handleSendReply = async () => {
    ...
    try {
      ...
      // Translate reply content if needed
      let translatedReply = replyContent;
      if (lang !== 'en') {
        try {
          translatedReply = await translateContent(replyContent, lang);
        } catch (err) {
          console.error('Error translating reply:', err);
        }
      }
  ...
  const handleRefresh = async () => {
    setIsLoading(true);
    try {
      const refreshed = await getMessages(userEmail, lang);
      setMessages(refreshed);
      toast.success('Messages refreshed!');
    } catch (error) {
