...
//       // Reset stats to default values when no data exists
//       setStats({
//         totalProperties: 0,
//         activeProperties: 0,
//         totalBookings: 0,
...
//     // FIX: Only sum bookings with status "confirmed"
//     const totalEarnings = bookings
//       .filter((b) => statusMatches(b.status, ["confirmed", "confirmé", "byemejwe"]))
//       .reduce((sum, b) => sum + (b.totalAmount || 0), 0);
    
//     // FIX: Handle average rating safely - prevent undefined
//     let averageRating = 0;
//     if (houses.length > 0) {
//       const totalRating = houses.reduce((sum, h) => {
//         // Ensure rating is a number, default to 0 if undefined or null
//         const rating = typeof h.rating === 'number' ? h.rating : 0;
//         return sum + rating;
//       }, 0);
//       averageRating = totalRating / houses.length;
//     }
...
//     // Only include confirmed bookings in earnings trend
...
//       // Only add earnings for confirmed bookings
//       if (statusMatches(booking.status, ["confirmed", "confirmé", "byemejwe"])) {
//         monthlyMap[monthKey].earnings += booking.totalAmount || 0;
//       }
//     });
...
export const HostDashboard: React.FC = () => {
  ...
  // Translate data function using MyMemory API
  const translateData = async (housesData: House[], bookingsData: Booking[]) => {
    if (lang === "en") {
      return { housesData, bookingsData };
    }
  ...
  useEffect(() => {
    ...
    const totalBookings = bookings.length;
    // Only sum bookings with status "confirmed"
    const totalEarnings = bookings
      .filter((b) => statusMatches(b.status, ["confirmed", "confirmé", "byemejwe"]))
      .reduce((sum, b) => sum + (b.totalAmount || 0), 0);
    
    // Handle average rating safely - prevent undefined
    let averageRating = 0;
    if (houses.length > 0) {
      const totalRating = houses.reduce((sum, h) => {
        // Ensure rating is a number, default to 0 if undefined or null
            ...
            <div>
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    {t.welcome}, {user?.name || t.hostUser}!
                  </p>
                </div>
              </div>
            </div>
