...
// // Manager restricted roles - only host and user
// type ManagerAllowedRole = "user" | "host";
...
//   // Check if user role is allowed for manager operations
...
//     manageUsers: "Manage users and hosts",
//     addUser: "Add User",
//     total: "Total",
//     active: "Active",
//     inactive: "Inactive",
...
//     viewUser: "View User",
//     notAuthorized: "You are not authorized to manage admin or manager accounts",
//   },
//   fr: {
//     userManagement: "Gestion des Utilisateurs",
//     manageUsers: "Gérer les utilisateurs et les hôtes",
...
//     notAuthorized: "Vous n'êtes pas autorisé à gérer les comptes administrateur ou gestionnaire",
//   },
//   rw: {
//     userManagement: "Gucunga Abakoresha",
//     manageUsers: "Gucunga abakoresha n'abatunze inzu",
...
//     notAuthorized: "Ntabwo uremewe gucunga abayobozi cyangwa abagenzuzi",
//   }
// };
...
//   // Form states - restricted to host and user only
...
//   // Fetch users from API with data sanitization - only users with host and user roles
...
//         // Sanitize user data and filter to only show host and user roles
...
//             },
//           }))
//           // Filter to only show host and user roles (manager can only see these)
//           .filter((user: User) => safeUserHelpers.isRoleAllowed(user.role));
...
//   // Open edit modal with user data - only if role is allowed
...
//       password: "",
//       confirmPassword: "",
//       role: user.role as ManagerAllowedRole,
//       isActive: user.isActive !== undefined ? user.isActive : true,
//     });
...
//       {/* Stats Cards - Manager specific: removed Admins and Managers stats */}
...
//       {/* Create User Modal - Manager restricted to host and user roles */}
//       <AnimatePresence>
//         {isCreateModalOpen && (
//           <>
//             <motion.div
...
//                   {/* Role & Status Row - Manager restricted to host and user */}
...
//                           handleInputChange(
//                             "role",
//                             e.target.value as ManagerAllowedRole,
//                           )
//                         }
...
//       {/* Edit User Modal - Manager restricted to host and user roles */}
//       <AnimatePresence>
//         {isEditModalOpen && (
//           <>
//             <motion.div
...
const safeUserHelpers = {
  ...
  formatRole: (user: User | null | undefined, translations?: any): string => {
    if (!user || !user.role || user.role.length === 0)
      return translations?.user || "User";
    const roleMap: Record<string, string> = {
      admin: translations?.admin || "Admin",
      user: translations?.user || "User",
      host: translations?.hosts || "Host",
      manager: translations?.manager || "Manager",
    };
    return (
      roleMap[user.role] ||
  ...
  formatStatus: (user: User | null | undefined, translations?: any): string => {
    if (!user) return translations?.active || "Active";
  ...
  // Get translated role for display
  getTranslatedRole: (role: string, translations: any): string => {
    const roleMap: Record<string, string> = {
      admin: translations?.admin || "Admin",
      user: translations?.user || "User",
      host: translations?.hosts || "Host",
  ...
  getTranslatedStatus: (isActive: boolean, translations: any): string => {
    return isActive
      ? translations?.active || "Active"
      : translations?.inactive || "Inactive";
  },
...
const ViewUserModal: React.FC<ViewUserModalProps> = ({
  ...
  // Safe user data access with translations
  const userInitial = safeUserHelpers.getInitial(user);
  const userDisplayName = safeUserHelpers.getDisplayName(user);
  const userDisplayEmail = safeUserHelpers.getDisplayEmail(user);
  const userDisplayRole = safeUserHelpers.getDisplayRole(user);
  ...
  // Translate status
  const getTranslatedStatus = (isActive: boolean): string => {
    return isActive ? translations.active : translations.inactive;
  };
          ...
          >
            <div className="w-full max-w-[95%] sm:max-w-md md:max-w-lg lg:max-w-xl xl:max-w-2xl 2xl:max-w-3xl rounded-2xl shadow-2xl bg-white relative overflow-hidden mx-2">
              ...
              <div className="p-4 sm:p-6 max-h-[80vh] sm:max-h-[70vh] overflow-y-auto">
                ...
                <div className="flex flex-col items-center mb-6">
                  <motion.div
                    className="w-16 h-16 sm:w-20 sm:h-20 md:w-24 md:h-24 rounded-full bg-[#FF385C] text-white flex items-center justify-center text-2xl sm:text-3xl font-bold mb-3"
                    whileHover={{ scale: 1.1, rotate: 10 }}
                    transition={{ duration: 0.3 }}
                  >
                    {userInitial}
                  ...
                  <div className="flex flex-wrap justify-center gap-2 mt-2">
                    <span
                      className={`px-2 sm:px-3 py-1 text-xs font-medium rounded-full ${getRoleColor(
                        userDisplayRole,
                      )}`}
                    >
                      {getTranslatedRole(userDisplayRole)}
                    </span>
                    <span
                    ...
                    >
                      {getTranslatedStatus(user.isActive)}
                    </span>
                    {user.isEmailVerified && (
                ...
                <div className="border-t border-gray-200 pt-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    {user.lastLogin && (
                      <div className="flex justify-between py-2 col-span-1 sm:col-span-2">
                        <span className="text-sm text-gray-500">
                ...
                <motion.button
                  ...
                  whileTap={{ scale: 0.98 }}
                  onClick={onClose}
                  className="w-full mt-6 py-2.5 rounded-lg bg-[#FF385C] text-white font-medium hover:bg-[#E31C5F] transition-colors text-sm sm:text-base"
                >
                  {t.close || "Close"}
                </motion.button>
              </div>
...
const translations = {
  ...
  fr: {
    ...
    viewUser: "Voir l'Utilisateur",
    notAuthorized:
      "Vous n'êtes pas autorisé à gérer les comptes administrateur ou gestionnaire",
  ...
  rw: {
    ...
    admin: "Ubuyobozi",
    manager: "Ugenzuzi",
    emailVerified: "Yemejwe",
    userId: "Indangamuntu",
    updated: "Yavuguruwe",
    lastLogin: "Yinjiye",
    statistics: "Ibibarwa",
    totalIncome: "Amahera Yose",
    totalExpenses: "Amakoreshwa Yose",
    totalSavings: "Ibikize Yose",
    close: "Funga",
    previous: "Mbere",
    next: "Ikurikira",
    page: "Urupapuro",
...
export const ManagerUserManagement: React.FC = () => {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        const total = Math.ceil(sanitizedUsers.length / itemsPerPage);
  ...
  }, [users, searchTerm, filterRole, filterStatus, itemsPerPage]);
  ...
  const toggleSelectAll = () => {
    ...
    if (allSelected) {
      setSelectedUsers(
        selectedUsers.filter((id) => !currentUserIds.includes(id)),
      );
    } else {
  ...
  // Get translated role for display in table
  const getTranslatedRole = (role: string): string => {
    return safeUserHelpers.getTranslatedRole(role, t);
  };

  // Get translated status for display in table
  const getTranslatedStatus = (isActive: boolean): string => {
    return safeUserHelpers.getTranslatedStatus(isActive, t);
  };
          ...
          <>
            ...
            <div className="px-4 py-3 border-t border-gray-200 bg-gray-50 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              <div className="text-sm text-gray-500">
                {t.showing} {getCurrentPageUsers().length} {t.of}{" "}
                {filteredUsers.length} {t.usersCount}
                {filteredUsers.length > 0 && (
                  ...
                  {totalPages > 5 && currentPage < totalPages - 2 && (
                    <>
                      ...
                      <button
                        ...
                        className="px-3 py-1 rounded-lg text-sm text-gray-600 hover:bg-gray-100 transition-colors"
                      >
                        {totalPages}
                      </button>
                    </>
