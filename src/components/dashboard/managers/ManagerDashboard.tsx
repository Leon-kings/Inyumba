...
//   const [showAllHousesModal, setShowAllHousesModal] = useState(false);
...
//   // Get total revenue from booking stats
...
//   // Generate user growth data from API - using actual months
...
//     const growthPattern = [0.05, 0.08, 0.12, 0.15, 0.25, 0.35];
...
//     return userStats.recentUsers.map((user, index) => {
//       const now = new Date();
//       const createdAt = new Date(user.createdAt);
//       const diffMs = now.getTime() - createdAt.getTime();
//       const diffMins = Math.floor(diffMs / 60000);
...
//   // Modal component for Recent Activities
//   const ActivitiesModal = () => {
//     if (!showAllActivitiesModal) return null;
...
//                   <div
//                     key={activity.id}
//                     className="py-4 flex items-center gap-4 hover:bg-gray-50 transition-colors px-2 rounded-lg"
//                   >
//                     <div
...
//   // Modal component for All Houses
//   const HousesModal = () => {
//     if (!showAllHousesModal) return null;
...
//             </h3>
//             <button
//               onClick={() => setShowAllHousesModal(false)}
//               className="p-1 hover:bg-gray-100 rounded-lg transition-colors"
//             >
//               <CloseIcon className="w-6 h-6 text-gray-500" />
//             </button>
...
//                         <span className="ml-1">{house.totalReviews || 0}</span>
//                       </div>
//                       <div>
//                         <span className="text-gray-400">Price:</span>
//                         <span className="ml-1 font-medium text-[#FF385C]">
//                           {house.revenue}
//                         </span>
//                       </div>
//                       <div>
//                         <span className="text-gray-400">Guests:</span>
//                         <span className="ml-1">{house.students}</span>
//                       </div>
//                       <div>
//                         <span className="text-gray-400">Bedrooms:</span>
//                         <span className="ml-1">{house.bedrooms}</span>
//                       </div>
//                       <div>
//                         <span className="text-gray-400">Bathrooms:</span>
//                         <span className="ml-1">{house.bathrooms}</span>
//                       </div>
//                     </div>
...
//                 {bookingStats && getTotalRevenue() > 0
...
//           {/* User Growth - Bar Chart with actual months */}
//           <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
//             <div className="flex items-center justify-between mb-4">
//               <h3 className="text-lg font-semibold text-gray-900">
//                 {t.userGrowth}
...
//           {/* Top Performers - Now showing 5 items */}
...
//                 </h3>
//                 <button
//                   onClick={() => setShowAllHousesModal(true)}
//                   className="text-sm text-[#FF385C] hover:underline"
//                 >
//                   {t.viewAll}
//                 </button>
...
const translations = {
  en: {
    ...
    total: "Total",
    allHousesTitle: "All Houses",
    allActivitiesTitle: "All Activities",
  },
  fr: {
    dashboard: "Tableau de Bord",
    adminDashboard: "Tableau de Bord Gestionnaire",
    overview: "Aperçu",
    users: "Utilisateurs",
    houses: "Maisons",
    bookings: "Réservations",
    ...
    total: "Total",
    allHousesTitle: "Toutes les Maisons",
  ...
  rw: {
    ...
    total: "Byose",
    allHousesTitle: "Amazu Yose",
    allActivitiesTitle: "Ibikorwa Byose",
  },
};
...
export const ManagersDashboard: React.FC = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : t.failedToFetch;
      setError(errorMessage);
      toast.error(errorMessage);
    } finally {
      setLoading(false);
  ...
  const getRecentActivitiesFromApi = () => {
    ...
    return recentUsers.map((user, index) => {
      ...
      let timeAgo = t.justNow;
      if (diffDays > 0) {
        timeAgo = `${diffDays}${t.daysAgo}`;
      } else if (diffHours > 0) {
        timeAgo = `${diffHours}${t.hoursAgo}`;
      } else if (diffMins > 0) {
        timeAgo = `${diffMins}${t.minutesAgo}`;
      }

      const action = user.isActive
        ? t.activeUserAccountCreated
        : t.userAccountCreatedInactive;
                ...
                {allActivities.map((activity) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      >
                        {activity.isActive ? t.active : t.inactive}
                      </span>
                      <span className="text-xs px-2 py-1 rounded-full bg-blue-100 text-blue-700">
                        {activity.role}
                      </span>
                ...
                {allHouses.map((house, index) => (
                  ...
                  >
                    ...
                    <div className="grid grid-cols-2 gap-2 text-sm text-gray-600">
                      ...
                      <div>
                        <span className="text-gray-400">{t.reviews}:</span>
                        <span className="ml-1">{house.totalReviews || 0}</span>
                      </div>
                      <div>
                      ...
                      <div>
                        <span className="text-gray-400">{t.bedrooms}:</span>
                        <span className="ml-1">{house.bedrooms}</span>
                      </div>
                      <div>
                        <span className="text-gray-400">{t.bathrooms}:</span>
                        <span className="ml-1">{house.bathrooms}</span>
                      </div>
                    </div>
                    <div className="mt-2 text-xs text-gray-400">
                    ...
                    <div className="mt-2 flex items-center gap-2">
                      ...
                      >
                        {house.isActive ? t.active : t.inactive}
                      </span>
                      <span className="text-xs text-gray-400">
                        +{house.growth}% {t.growth}
                      </span>
                    </div>
                  </div>
                ))}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-sm text-purple-500 font-medium">
                {bookingStats
                  ? t.totalBookingsCount.replace("{count}", bookingStats.data.total.toString())
                  : "0 bookings"}
              </span>
            </div>
            <div className="h-64">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-sm text-blue-500 font-medium">
                {userStats && userStats.statistics.newUsersLast30Days > 0
                  ? t.newUsersCount.replace("{count}", userStats.statistics.newUsersLast30Days.toString())
                  : "+0"}
              </span>
            </div>
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={userGrowthData}>
                  ...
                  <Tooltip
                    ...
                    formatter={(value) => [`${value} ${t.users}`, t.users]}
                  />
                  <Bar dataKey="users" fill="#4F46E5" radius={[8, 8, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
              ...
              <div className="divide-y divide-gray-100">
                {topPerformers.map((performer, index) => (
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between text-xs text-gray-500">
                      <span>
                        {performer.students} {t.guests}
                      </span>
                      <span>
                        {performer.houses} {t.housesLabel}
                      </span>
                      <span className="font-medium text-gray-900">
                        {performer.revenue}
                      </span>
