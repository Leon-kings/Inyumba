...
export const ManagersDashboard: React.FC = () => {
  ...
  const getUserGrowthData = () => {
    ...
    // Get actual month names based on current date
    const monthNames = [
    ...
    // Distribute users across months based on actual data
    // Use a more realistic distribution pattern
    const baseUsers = Math.max(0, totalUsers - newUsers);
  ...
  const getTopPerformersFromApi = () => {
    ...
    // Sort houses by rating and get top 5
    const topHouses = [...houseData.data]
      .sort((a, b) => b.rating - a.rating)
      .slice(0, 5);
                  ...
                  >
                    ...
                    <div className="grid grid-cols-2 gap-2 text-sm text-gray-600">
                      <div>
                        ...
                        <span className="ml-1 font-medium">★ {house.rating.toFixed(1)}</span>
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <span className="ml-1 font-medium text-[#FF385C]">{house.revenue}</span>
                      </div>
                      <div>
                    ...
                    <div className="mt-2 text-xs text-gray-400">
                      {house.location?.province && (
                        <span>{house.location.province}, {house.location?.district || ''}</span>
                      )}
                    </div>
                    <div className="mt-2 flex items-center gap-2">
                      <span className={`text-xs px-2 py-1 rounded-full ${
                        house.isActive ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'
                      }`}>
