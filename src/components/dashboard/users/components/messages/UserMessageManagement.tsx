...
export const UserMessageManagement: React.FC = () => {
  ...
  const fetchMessages = async () => {
    ...
    try {
      ...
      showErrorModal(t.error || "Error", t.fetchError || "Failed to load messages");
    } finally {
      setIsFetching(false);
    }
  };
  ...
  const handleCreateMessage = async () => {
    ...
    if (!isValid) {
      showErrorModal(t.error || "Error", t.pleaseFixErrors || "Please fix the errors above");
      setTouchedFields({
        name: true,
        email: true,
        message: true,
                ...
                filteredMessages.map((message) => (
                  ...
                  >
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        <div className="flex-shrink-0 flex items-center gap-1">
                          {message.isStarred && <StarIcon className="w-4 h-4 text-yellow-400 fill-current" />}
                          {message.isFlagged && <FlagIcon className="w-4 h-4 text-red-400 fill-current" />}
                        </div>
                        <div className="min-w-0">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-center gap-1">
                        <motion.button
                          whileHover={{ scale: 1.1 }}
                          whileTap={{ scale: 0.9 }}
                          onClick={(e) => { e.stopPropagation(); openViewModal(message); }}
                          className="p-1.5 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors"
                          title={t.viewMessage}
                        >
                          <VisibilityIcon className="w-4 h-4" />
                        ...
                        <motion.button
                          whileHover={{ scale: 1.1 }}
                          whileTap={{ scale: 0.9 }}
                          onClick={(e) => { e.stopPropagation(); openReplyModal(message); }}
                          className="p-1.5 text-green-600 hover:bg-green-50 rounded-lg transition-colors"
                          title={t.reply}
                        >
                          <ReplyIcon className="w-4 h-4" />
                        ...
                        <motion.button
                          whileHover={{ scale: 1.1 }}
                          whileTap={{ scale: 0.9 }}
                          onClick={(e) => { e.stopPropagation(); handleToggleStar(message._id); }}
                          ...
                          title={t.toggleStar}
                        >
                          {message.isStarred ? <StarIcon className="w-4 h-4" /> : <StarBorderIcon className="w-4 h-4" />}
                        </motion.button>
                        <motion.button
                          whileHover={{ scale: 1.1 }}
                          whileTap={{ scale: 0.9 }}
                          onClick={(e) => { e.stopPropagation(); handleToggleFlag(message._id); }}
                          ...
                          title={t.toggleFlag}
                        >
                          {message.isFlagged ? <FlagIcon className="w-4 h-4" /> : <FlagOutlinedIcon className="w-4 h-4" />}
                        </motion.button>
                        <motion.button
                          whileHover={{ scale: 1.1 }}
                          whileTap={{ scale: 0.9 }}
                ...
                <div className="sticky top-0 px-6 py-4 flex items-center justify-between border-b border-gray-200 bg-white/95 backdrop-blur-sm rounded-t-2xl z-10">
                  ...
                  <div className="flex items-center gap-2">
                    <motion.button
                      ...
                      className="p-1.5 rounded-full hover:bg-gray-100 transition-colors"
                    >
                      {selectedMessage.isStarred ? <StarIcon className="w-5 h-5 text-yellow-400" /> : <StarBorderIcon className="w-5 h-5 text-gray-400" />}
                    </motion.button>
                    <motion.button
                      whileHover={{ scale: 1.1 }}
                      whileTap={{ scale: 0.9 }}
                      ...
                      className="p-1.5 rounded-full hover:bg-gray-100 transition-colors"
                    >
                      {selectedMessage.isFlagged ? <FlagIcon className="w-5 h-5 text-red-400" /> : <FlagOutlinedIcon className="w-5 h-5 text-gray-400" />}
                    </motion.button>
                    <motion.button
                      whileHover={{ rotate: 90, scale: 1.1 }}
                      whileTap={{ scale: 0.9 }}
                ...
                <div className="p-6 space-y-4">
                  ...
                  <div className="flex flex-wrap gap-3 pt-4 border-t border-gray-200">
                    <motion.button
                      whileHover={{ scale: 1.02 }}
                      whileTap={{ scale: 0.98 }}
                      onClick={() => { setIsViewModalOpen(false); openReplyModal(selectedMessage); }}
                      className="px-4 py-2.5 bg-[#FF385C] text-white rounded-lg font-medium hover:bg-[#E31C5F] transition-colors flex items-center gap-2"
                    >
                      <ReplyIcon className="w-4 h-4" />
                      {t.reply}
                    ...
                    <motion.button
                      ...
                      className="px-4 py-2.5 border border-gray-300 rounded-lg text-gray-700 font-medium hover:bg-gray-50 transition-colors flex items-center gap-2"
                    >
                      {selectedMessage.status === "archived" ? <UnarchiveIcon className="w-4 h-4" /> : <ArchiveIcon className="w-4 h-4" />}
                      {selectedMessage.status === "archived" ? t.unarchive : t.archive}
                    </motion.button>
                    <motion.button
                      whileHover={{ scale: 1.02 }}
                      whileTap={{ scale: 0.98 }}
                      onClick={() => { setIsViewModalOpen(false); openDeleteModal(selectedMessage); }}
                      className="px-4 py-2.5 border border-red-300 text-red-600 rounded-lg font-medium hover:bg-red-50 transition-colors flex items-center gap-2"
                    >
                      <DeleteIcon className="w-4 h-4" />
                      {t.delete}
              ...
              <div className="w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl shadow-2xl bg-white relative">
                <div className="sticky top-0 px-6 py-4 flex items-center justify-between border-b border-gray-200 bg-white/95 backdrop-blur-sm rounded-t-2xl z-10">
                  ...
                  <motion.button
                    whileHover={{ rotate: 90, scale: 1.1 }}
                    whileTap={{ scale: 0.9 }}
                    onClick={() => { setIsReplyModalOpen(false); setReplyContent(""); }}
                    className="p-1.5 rounded-full transition-colors hover:bg-gray-100 text-gray-500"
                  >
                    <CloseIcon className="w-5 h-5" />
                  </motion.button>
        ...
        {isComposeModalOpen && (
          <>
            <motion.div
              ...
              exit="exit"
              className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100]"
              onClick={() => { setIsComposeModalOpen(false); resetForm(); }}
            />
            <motion.div
              variants={modalVariants}
              initial="hidden"
              ...
              <div className="w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl shadow-2xl bg-white relative">
                ...
                <div className="p-6 space-y-4">
                  {isFormValid && touchedFields.name && touchedFields.email && touchedFields.message && (
                    <div className="p-3 bg-green-50 border border-green-200 rounded-lg flex items-center gap-2">
                      <CheckCircleIcon className="w-5 h-5 text-green-600" />
                      <span className="text-sm text-green-700 font-medium">{t.allFieldsValid}</span>
                    </div>
                  )}
                  ...
                  {!isFormValid && (touchedFields.name || touchedFields.email || touchedFields.message) && (
                    <div className="p-3 bg-red-50 border border-red-200 rounded-lg flex items-center gap-2">
                      <ErrorIcon className="w-5 h-5 text-red-600" />
                      <span className="text-sm text-red-700 font-medium">{t.pleaseFixErrors}</span>
                    </div>
                  )}
                    ...
                    <div className="relative">
                      <textarea
                        ...
                        className={`w-full px-3 py-2.5 border rounded-lg focus:ring-2 focus:ring-[#FF385C] focus:border-transparent outline-none text-sm resize-none transition-colors ${
                          touchedFields.message && formErrors.message
                            ? "border-red-500 bg-red-50"
                            : touchedFields.message && !formErrors.message && formData.message
                              ? "border-green-500 bg-green-50"
                              : "border-gray-300"
                        }`}
                        placeholder="Type your message here..."
