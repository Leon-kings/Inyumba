...
export const UserMessageManagement: React.FC = () => {
  ...
  // ===== FIXED: Add ref for delete target =====
  const deleteTargetRef = useRef<Message | null>(null);
  // ===== END FIXED =====
  ...
  // ===== FIXED: Delete function using ref =====
  const handleDeleteMessage = async () => {
    const messageToDelete = deleteTargetRef.current;
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/contact/${messageToDelete._id}`,
        {
          method: "DELETE",
        },
      );
      ...
      showSuccessModal(
        t.success || "Success!",
        t.messageDeleted || "Message deleted successfully!",
        `Message from ${messageToDelete.senderName} has been removed`,
      );

      closeConfirmModal();
      setIsViewModalOpen(false);
      setSelectedMessage(null);
      deleteTargetRef.current = null; // Clear the ref
    } catch (error) {
  ...
  // ===== FIXED: Delete modal opener using ref =====
  const openDeleteModal = (message: Message) => {
    deleteTargetRef.current = message; // Store in ref
    showConfirmModal(
      "⚠️ " + (t.deleteMessage || "Delete Message"),
      `${t.deleteConfirmation || "Are you sure you want to delete this message?"} ${t.actionUndone || "This action cannot be undone."}`,
      handleDeleteMessage,
